...
{
  pthread_mutex_lock(&thpool_p->thcount_lock);
  int num = thpool_p->num_threads_working;
  pthread_mutex_unlock(&thpool_p->thcount_lock);
...
long
thpool_num_jobs_placed(thpool_* thpool_p)
{
  return thpool_p->num_jobs_placed;
}

long
thpool_num_jobs_done(thpool_* thpool_p)
{
...
#if defined(__linux__)
  /* Use prctl to avoid requiring _GNU_SOURCE flag and implicit declaration */
